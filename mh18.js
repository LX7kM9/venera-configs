...
class MH18 extends ComicSource {
  ...
  version = "1.0.1"   // 增加复制链接和链接解析跳转功能

  minAppVersion = "1.4.0"

  // update url
  url = "https://cdn.jsdelivr.net/gh/LX7kM9/venera-configs@main/mh18.js"
  ...
  comic = {
    ...
    link: {
      ...
      linkToId: (url) => {
        // 提取 /manga/xxx 或 /manga-xxx 等路径（漫画详情页路径）
        let match = url.match(/\/manga[^/]*(?:\/[^/]+)?/);
        if (match) return match[0];
        // 备用：提取任何以 /manga 开头的路径
        match = url.match(/\/manga[^/]*/);
