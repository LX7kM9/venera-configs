class Comick extends ComicSource {
    name = "comick"
    key = "comick"
    version = "1.3.0"   // 增加复制链接和链接解析功能
    minAppVersion = "1.4.0"
    // update url
    url = "https://cdn.jsdelivr.net/gh/LX7kM9/venera-configs@main/comick.js"
    ...
    comic = {
        ...
        loadInfo: async (id) => {
            ...
            if (id.includes("//")) {
                let parts = id.split("//");
                cId = parts[0];
                cTitle = parts.slice(1).join("//"); // 可能标题中含有 //
            } else {
                cId = id;
                cTitle = "";
            }
            ...
            // ========== 生成详情页 URL（供复制链接使用） ==========
            const comicUrl = `https://comick.art/comic/${cId}`;
        ...
        link: {
            ...
            /**
             * 从 URL 中提取漫画 ID（即 slug）
             * @param {string} url - 完整的漫画详情页链接
             * @returns {string|null} - 漫画 slug，若无法解析则返回 null
             */
            linkToId: (url) => {
                // 匹配 /comic/ 后面的 slug（不含 / 和 ?）
