class Wnacg extends ComicSource {
    ...
    version = "1.0.6"   // 增加复制链接和链接解析跳转功能

    minAppVersion = "1.0.0"

    // update url
    url = "https://cdn.jsdelivr.net/gh/LX7kM9/venera-configs@main/wnacg.js"
    ...
    comic = {
        ...
        loadInfo: async (id) => {
            ...
            // ===== 新增：构造详情页链接 =====
            let detailUrl = `${this.baseUrl}/photos-index-page-1-aid-${id}.html`;
        ...
        // ===== 新增：链接解析与跳转（动态获取域名） =====
        get link() {
            // 收集所有可用域名：Wnacg.domains 加上当前自定义域名（如果有）
            let allDomains = [];
            // 添加 Wnacg.domains 中的域名
            for (let d of Wnacg.domains) {
                if (d && !allDomains.includes(d)) allDomains.push(d);
            }
            // 添加自定义域名（如果设置）
            try {
                let custom = this.loadSetting('domain0');
                if (custom && custom.trim() && !allDomains.includes(custom.trim())) {
                    allDomains.push(custom.trim());
                }
            } catch (e) {}
            // 如果没有域名，至少提供一个默认值
            if (allDomains.length === 0) {
                allDomains.push('wnacg.com');
            }

            return {
                domains: allDomains,
                linkToId: (url) => {
                    // 匹配 -aid-数字 或 aid=数字 或 /aid/数字
                    let match = url.match(/[-?&/]aid[-=](\d+)/);
                    if (match) return match[1];
                    // 匹配 photos-index-page-1-aid-数字.html 等
                    match = url.match(/-aid-(\d+)/);
                    if (match) return match[1];
                    // 匹配 /photos-gallery-aid-数字.html
                    match = url.match(/\/photos-gallery-aid-(\d+)/);
                    if (match) return match[1];
                    // 匹配数字结尾（如 .../123.html）
