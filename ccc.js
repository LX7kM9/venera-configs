...
class CCC extends ComicSource {
    ...
    version = "1.0.9"   // 支持将 reader_comic 的 ID 作为章节 ID

    minAppVersion = "1.6.0"

    // update url
    url = "https://cdn.jsdelivr.net/gh/LX7kM9/venera-configs@main/ccc.js"
    ...
    comic = {
        ...
        loadInfo: async (id) => {
            // 先尝试正常获取 info
            let res = await Network.get(`${this.apiUrl}/book/${id}/info`, await this.getApiHeaders());
            let jsonData = null;
            let isSpecial = false;
            let chapters = {};
            ...
            if (res.status === 200) {
                try {
                    ...
                    if (body.code === 0 && body.data) {
                        ...
                        throw body.message || "未知错误";
                    }
                } catch (e) {
                    throw `解析数据失败: ${e.message || e}`;
                }
            } else if (res.status === 403) {
                // info 403，尝试获取 chapter 列表
                const chapterRes = await Network.get(`${this.apiUrl}/book/${id}/chapter`, await this.getApiHeaders());
                if (chapterRes.status === 200) {
                    const chapterBody = JSON.parse(chapterRes.body);
                    if (chapterBody.code === 0 && chapterBody.data) {
                        const chapterData = chapterBody.data;
                        jsonData = {
                            name: chapterData?.book?.name || `作品 ${id}`,
                            brief: chapterData?.book?.brief || "",
                            description: "",
                            image1: chapterData?.book?.image1 || "",
                            image2: chapterData?.book?.image2 || "",
                            image3: chapterData?.book?.image3 || "",
                            type: { name: "特殊" },
                            author: chapterData?.book?.author || [],
                            tags: [],
                            is_collected: 0,
                            updated_at: "",
                            like_count_only_uuid: 0,
                        };
                        ...
                        throw chapterBody.message || "获取章节信息失败";
                    }
                } else {
                    // info 和 chapter 都 403，尝试将该 ID 作为章节 ID
                    const epRes = await Network.get(`${this.apiUrl}/book/chapter/${id}`, await this.getApiHeaders());
                    if (epRes.status === 200) {
                        // 该 ID 是一个有效的章节 ID，构造单章节详情页
                        const epData = JSON.parse(epRes.body);
                        const bookInfo = epData?.data?.book || {};
                        const chapterInfo = epData?.data?.chapter || {};
                        jsonData = {
                            name: bookInfo?.name || `章节 ${id}`,
                            brief: chapterInfo?.name || "",
                            description: "",
                            image1: bookInfo?.image1 || "",
                            image2: bookInfo?.image2 || "",
                            image3: bookInfo?.image3 || "",
                            type: { name: "章节" },
                        ...
                        // 完全无法获取
                        // 尝试从 reader_comic 页面提取标题和封面（最后备用）
                        const readerUrl = `https://www.creative-comic.tw/zh/reader_comic/${id}`;
                        let title = `作品 ${id}`;
                        let cover = "";
                        try {
                            const readerRes = await Network.get(readerUrl, {
                                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
                            });
                            if (readerRes.status === 200) {
                                const html = readerRes.body;
                                const titleMatch = html.match(/<title>(.*?)<\/title>/);
                                if (titleMatch && titleMatch[1]) {
                                    let rawTitle = titleMatch[1].trim();
                                    rawTitle = rawTitle.replace(/\s*[-|–]\s*CCC.*$/, "").trim();
                        ...
                        // 插入虚拟章节，但这次使用 "reader" 字符串，保留备用（但通常不会触发）
                        chapters = { "reader": "开始阅读" };
                        isSpecial = true;
                    }
                }
            } else {
                throw `获取漫画信息失败 (HTTP ${res.status})`;
            }
            ...
            // 如果章节尚未填充（正常情况），则正常获取（但上面已经处理了）
            if (Object.keys(chapters).length === 0 && !isSpecial) {
            ...
            // 构造详情页 URL
            const comicUrl = isSpecial
                ? `https://www.creative-comic.tw/zh/reader_comic/${id}`
                : `https://www.creative-comic.tw/zh/book/${id}`;
        ...
        loadEp: async (comicId, epId) => {
            // 如果 epId 是数字，直接请求章节 API
            if (epId !== "reader" && /^\d+$/.test(epId)) {
            ...
            // 虚拟章节（"reader"）——保留备用，但一般不会走到这里
            if (epId === "reader") {
                const readerUrl = `https://www.creative-comic.tw/zh/reader_comic/${comicId}`;
                try {
                    const res = await Network.get(readerUrl, {
                        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
                    });
                    if (res.status === 200) {
                        const html = res.body;
                        ...
                        const seen = new Set();
                        // 正则提取图片 URL
                        const imgRegex = /<(?:img|amp-img)[^>]+(?:src|data-src|data-original)=["']([^"']+)["']/gi;
                        let match;
                        while ((match = imgRegex.exec(html)) !== null) {
                            let url = match[1];
                            if (url.startsWith("//")) url = "https:" + url;
                            else if (url.startsWith("/")) url = "https://www.creative-comic.tw" + url;
                            if (/favicon|logo|avatar|icon|banner|btn|arrow|menu|share|social/i.test(url)) continue;
                            if (url.includes("storage.googleapis.com/ccc-www/fs/")) {
                                if (!seen.has(url)) {
                                    seen.add(url);
                                    imageUrls.push(url);
                                }
                ...
                UI.showMessage("未能提取到漫画图片，请检查网络");
                return { images: [] };
            }
        ...
        link: {
             ...
             * @returns {string|null} - 漫画 ID，若无法解析则返回 null
             */
            linkToId: (url) => {
                // 匹配 /book/ 或 /reader_comic/ 后面的数字
                let match = url.match(/\/(?:book|reader_comic)\/(\d+)/);
