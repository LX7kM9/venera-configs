...
class Lanraragi extends ComicSource {
    name = "Lanraragi"
    key = "lanraragi"
    version = "1.2.3"   // 修复搜索参数类型错误（布尔值 vs 字符串）
    minAppVersion = "1.4.0"
    url = "https://cdn.jsdelivr.net/gh/LX7kM9/venera-configs@main/lanraragi.js"
    ...
    search = {
        load: async (keyword, options, page = 1) => {
            ...
            // ===== 修复：转换为布尔值 =====
            const newonly = pick(2, 'false') === 'true'
            const untaggedonly = pick(3, 'false') === 'true'
            const groupby = pick(4, 'true') === 'true'
    ...
    comic = {
        loadInfo: async (id) => {
            ...
            const cover = `${this.baseUrl}/api/archives/${id}/thumbnail`
                ...
                const detailUrl = `${this.baseUrl}/view/${id}`;
        ...
        link: {
            domains: ['lrr.tvc-16.science', 'localhost', '127.0.0.1'],
            linkToId: (url) => {
                let path = url.replace(/^https?:\/\/[^\/]+/, '');
                let match = path.match(/[\/\?&]view[=\/]([^\/\&\?]+)/);
                if (match) return match[1];
                match = path.match(/\/archives?\/([^\/\&\?]+)/);
                if (match) return match[1];
